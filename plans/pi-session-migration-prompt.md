...
# open herdr workspaces + live pi agents (labels platform-1-* / platform-2-*)
...
- **platform-1** master e2e repo — commit `527506d0` (`refactor(feature-workflow): pi sessions to
...
- **platform-2** master repo — commit `ee9cb9c` (`refactor(feature-workflow): pi sessions to native
  default store…`) is on platform-2 `main`: changed
  `scripts/feature-workflow/{feature-lib.sh,feature-start}`. Pull platform-2 `main`.
...
- If the script is unavailable on this host (e.g. no platform-1 repo present), reimplement its
...
Run it `--dry-run` first, review every target key (platform-1 + platform-2), then for real. Verify: zero
...
`platform-1-<name>` AND `platform-2-<name>` — confirm the agent is idle, kill its pi pid, then relaunch
...
- If the platform has `scripts/feature-workflow/feature-respawn` (platform-1 has it): use it.
- Otherwise (platform-2 does not have it), re-create the workspace directly:
...
- Do not assume only one platform exists: every feature-workflow copy found in Phase 1 must be
  fixed; every `platform-1-*`/`platform-2-*` workspace and feature root must be handled.
...
- Do not commit one repo's changes from another, or `git add` across repo boundaries; respect
  each repo's own git discipline (platform-1: glab/MRs toward `dev`; platform-2: gh/PRs toward `main`).
