# nvim-dbee plan — revised for platform-1 back-end repos

Goal: add `nvim-dbee` to this Neovim config in way that matches how `/home/ubby/Code/platform-1/back-end` actually works.
...
- **Node / Nest / Drizzle** repos
  - PostgreSQL via `DATABASE_URL` or `POSTGRES_*`: `repo-3`, `rule-engine`, `structure_v2`, `namager`
- **Symfony / PHP** repos
...
- **Schema matters in several repos**
  - examples: `task-mgmt`, `dealer-mgmt`, `mdm`, `repo-3`, `rule-engine`
...
Target repos like `repo-3`, `rule-engine`, `structure_v2`, `namager`.
...
- `repo-3` → Postgres via `DATABASE_URL`, schema `state_machine`
...
- add helper commands/scripts like `db-tunnel-task-mgmt-dev`, `db-tunnel-repo-3-test`
