...
|---|---|---|---|
| `Space+ee` | Open file explorer / project tree | `ActivateProjectToolWindow` | `Snacks.explorer()` |
...
| `Space+xx` | Diagnostics list | `ActivateProblemsViewToolWindow` / `Snacks.picker.diagnostics()` |
...
| Key | Action | IntelliJ backend | Neovim backend |
|---|---|---|---|
| `Space+db` | Database | `ActivateDatabaseToolWindow` | — (use DataGrip) |
| `Space+mv` | Maven | `ActivateMavenToolWindow` | — (use zellij bash pane) |
| `Space+pi` | AI Assistant | `ActivateAIAssistantToolWindow` | — (Pi in separate zellij pane) |
