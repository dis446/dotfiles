#!/usr/bin/env bash
# Shared helpers for the platform-1 feature workflow (feature-start/mr/stop/list).
#
# A "feature" = a directory under $FEATURES_ROOT (default ~/Code/<org>/<platform-1>/features)
...
ALPHA_ROOT="${ALPHA_ROOT:-$HOME/Code/<org>/<platform-1>}"
...
# Master (repo-1) checkout — the orchestrator/knowledge hub that
# owns the shared plans/, docs/, .agents/ dirs.
E2E_MAIN="$ALPHA_ROOT/back-end/repo-1"
...
# Resolve a repo argument (basename, env-name path, or absolute path)
...
# glab repo spec from a repo dir's origin remote (git@<gitlab-host>:platform-1/back-end/x.git)
glab_repo_of() {
