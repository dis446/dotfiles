...
---
Review GitLab MR $1
(https://<gitlab-host>/platform-1/back-end/bpm/repo-5/-/merge_requests/$1)
...
- Load matching skills from `.agents/skills/` by MR surface:
  `client-skill-ref` (definition/n8n traps), `repo-3-visual-layout`
  (positions/layout), `n8n-api` (workflow/execution diagnosis),
  `client-skill-ref` (spec — only if the client-skill-ref checkout is reachable; never
  guess the spec).
- Read `docs/client-doc.md` for the shared machinery the
...
- n8n: every N8N_TRIGGER path registered in `tenants/<tenant>/n8n/vendors.sh`
  ...
  workflows sourced + dispatched in `seed/n8n/client-skill-ref.sh`; borrowed mocks reused.
...
- New flows: plan doc in `docs/plans/`; seed sequencing documented (70-n8n
  before 60-repo-3; CLIENT_SM_ALLOW_CREATE=true on first SM seed, then the
