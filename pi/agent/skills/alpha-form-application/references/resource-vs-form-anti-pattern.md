...
This document is loaded by the parent `alpha-form-application` skill. It is **not** a standalone skill — no frontmatter, no independent trigger. Read it whenever the user's request implies a bespoke form over a data-model record, before instructing the planner.
...
See `../../alpha-form-resource-planner/SKILL.md` → "Resources vs. Forms — the core modeling decision" for the field-level shapes the planner emits.
