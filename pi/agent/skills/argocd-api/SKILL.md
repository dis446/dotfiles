...
ALPHA_ARGOCD_DEV_API_KEY=<jwt-placeholder-example>

# Test environment
ALPHA_ARGOCD_TEST_SERVER=argocd.test.example.com
ALPHA_ARGOCD_TEST_API_KEY=<jwt-placeholder-example>
```
...
ArgoCD API docs: <https://<gitlab-host>/swagger-ui>
...
- `namespace` is required; on these clusters app workloads live in **`platform-1`**.
...
argocd_logs contract-manager <pod> 800        # last 800 lines, namespace defaults to platform-1
...
# 2. fetch its logs
curl -s -H "Authorization: Bearer $ARGOCD_TOKEN" \
  "$ARGOCD_URL/api/v1/applications/contract-manager/pods/<pod>/logs?namespace=platform-1&tailLines=800&follow=false" \
