...
# Usage: argocd_logs <app> <pod> [tailLines=400] [namespace=platform-1] [container]
argocd_logs() {
  local app="$1" pod="$2" tail="${3:-400}" ns="${4:-platform-1}" container="${5:-}"
...
argocd_logs_all() {
  local app="$1" tail="${2:-400}" ns="${3:-platform-1}"
