...
Plugins observe every request the SDK issues against `baseUrl` — configure URLs before registering plugins so the plugin sees consistent endpoints. (`projectUrl` exists as SDK vocabulary but is inert in this stack: no project routing — use `getBaseUrl()`.)
...
const finalUrl = Formio.pluginAlter('rewriteUrl', `${Formio.getBaseUrl()}/intake`, {
  formId: 'intake',
});
```
...
Plugins are SDK-only — there are no MCP tools in this deployment. Use the SDK directly.
