...
The exact Phase B emission rules for `alpha-form-resource-planner`. Read this after the user approves the Resource Map and before emitting the `template.md` + `template.json` pair.
...
3. **Auth-form actions.** For the login form, add a Login action (`"<form>:login"`). For the register form, add a Role Assignment action (`"<form>:role"`) that assigns the default authenticated role, AND a Login action so the user is signed in immediately on successful registration. Execution order on the register form: Save (before, priority 10) → Login (before, priority 2) → Role Assignment (after, priority 1) — see "Priority ordering" below.
   ...
   **Admin-only work goes through the Form.io portal or CE admin API, not the app login form.** If the plan has administrator-only responsibilities (seeding reference data, assigning roles to users, reviewing submissions, moderating content), those are performed by an administrator using the CE admin portal or the CE admin API (`x-token` + `API_KEYS`, see `../_shared/stack.md`) — not the app's in-app login surface. Do NOT design the app's login surface around administrator access. Instead, capture the admin-only operations in `template.md`'s `## Users & Auth → Admin operations` line so downstream skills and the end user see which tasks need admin credentials.
...
| Join resource (`ProjectUser`, `TeamUser`) | `<join>:save` |
| Any form mentioned as sending email / webhook | the required set above PLUS `<name>:email` / `<name>:webhook` |

Priority ordering inside `actions`: execution is **descending by priority number within each handler phase** (higher number runs first). Save (priority 10) runs first, in the `before` phase — persist before downstream actions read the submission. Login (priority 2) also runs in the `before` phase, right after Save, issuing the JWT. Role Assignment (priority 1) runs in the `after` phase, after the submission is saved, attaching roles to the created record. So on a register form the execution order is Save → Login → Role Assignment. Use the priority values shown in [`template-json.md`](template-json.md); do not invent new numbers.
...
Then finish with a short "Next steps" section. This is a **menu you print**, not work you do: this skill writes the artifact pair and hands it back, and it does not run any of the calls below. Whoever does — `alpha-form-application` on its own flow, or you in a later turn once the user asks — owns the deployment gate for them, which means confirming the target CE base URL (`FORMIO_BASE_URL`) and the caller's credentials (`x-token` + `API_KEYS` for CE admin ops, or Keycloak gateway headers via the middleware `/v1`) before the first write. Never present the import as something already done.

```
### Next steps

- **Import the template**: `curl -X POST -H "x-token: $TOKEN" -H "Content-Type: application/json" -d "{\"template\": $(cat template.json)}" {FORMIO_BASE_URL}/import` — the CE bootstrap-style import (same endpoint `default-template.json` loads through). Per-tenant forms are created at runtime by the middleware (`/v1/form` with `x-api-gw-*` headers), which applies `tags`, `*_own` ACLs, and `{basePath}-{orgUuid}` path scoping — the template stays global/tenant-agnostic.
- **Or pass the pair to another skill / tool**: `alpha-form-application` consumes both files (Import step); framework scaffolders (future `formio-react`, React portals) — hand them BOTH `template.md` (architectural intent) and `template.json` (structured reference). The `.md` is the seed they reason from; the `.json` is the structured companion they consult for field-level shapes.
```
