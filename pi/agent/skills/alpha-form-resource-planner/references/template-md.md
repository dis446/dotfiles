...
Canonical shape for the Resource Map artifact emitted by `alpha-form-resource-planner` Phase B alongside `template.json`. Downstream skills (`alpha-form-application`, future `formio-react`, etc.) read `template.md` for architectural intent — what resources exist, how they relate, who can do what — and consult the paired `template.json` for field-level Form.io JSON shapes.
...
One block per resource. Terse — one sentence per purpose, one clause per field. Join resources are tagged inline.
...
- <JoinResourceName> (type: resource, join)
  ...
  Actions:
    - <join>:save — the join is plain membership/relationship data; no access action (no Group Assignment in this stack)
```
...
One block per form. Call out which Resource (if any) the form **references** (a record established earlier in the flow) and how. A bespoke form references an existing record — it never creates that record on submit; see `../SKILL.md` → "Resources vs. Forms" and `../../alpha-form-application/references/resource-vs-form-anti-pattern.md` → "Using Resources within Forms" for the anti-pattern.
...
- Admin operations: <each admin-only workflow in one line — e.g. "Seed initial Project rows; create ProjectUser membership rows; assign `administrator` to specific users." — noting they run through the CE admin portal or CE admin API (`x-token` + `API_KEYS`), not the app. `None.` when the app has none.>
...
- MDM: <existing `mdm-*` reference resources consumed via `dataSrc: "resource"` selects, or `none`>
```

The `Admin operations` line is what `phase-b-emission.md` → "Admin-only work goes through the portal or CE admin API, not the app login form" and `planning-rules.md` → "Login" both point at: admin-only responsibilities are captured here rather than designed into the app's login surface.

When `SSO` is `Keycloak` (gateway integration), or the plan needs admin auth beyond the CE `x-token`/`API_KEYS` story, tenant JWT mechanics, or RBAC tuning beyond default roles, downstream auth configuration is owned by the `alpha-form-auth` skill — hand off there after this Resource Map is approved.
...
A truth table of CRUD capability per resource × actor. Actors are roles (and the anonymous role where public). Cell values use this vocabulary — keep it small and consistent so consumers can match on exact strings:
...
There is no `group` token: no Group Assignment action exists in this stack (`../_shared/stack.md` → Absent).
...
Phase B verifies each matrix row against the arrays it emitted before handing off: for every row, for every column, confirm the mapping above actually appears.
...
| Project | authenticated | — | all | — | — | project list is read-only for members (populates the task form's project select) |
| Task | administrator | all | all | all | all |  |
| Task | authenticated | own | own | own | own | owner-scoped; the checked-in `examples/task-manager/` uses this exact shape |
| ProjectUser | administrator | all | all | all | all | admin-managed membership data |
...
Use a `%%` comment line above each cardinality to name the relationship when it is not obvious (e.g., `%% ProjectUser links users to projects`).

Example (owner-based access — Task Manager):
...
Mermaid `flowchart TD`. Shows how access actually reaches each resource at runtime — which role the user starts with, which `submissionAccess` rules grant which operations, and how admin bypass is modeled. This is the diagram the `Access Matrix` summarises and the `ER Diagram` does not show.
...
- Arrow labels name the mechanism: `"Role Assignment → <role>"`, `"Submission Access: read_own, update_own"`.
...
Example (Task Manager — owner-based):

````markdown
```mermaid
flowchart TD
    Admin[[administrator]] --> All[[every resource]]
    Auth[[authenticated]] -->|"Submission Access: read_own, update_own"| T[Task]
    Auth -->|"Submission Access: read_all"| P[Project]
```
````
...
`template.md` is consumed primarily by downstream skills and humans viewing the file in GitHub / IDE preview / Obsidian / any modern Markdown renderer. Mermaid scales beyond what ASCII handles cleanly (7+ resources, multiple joins) and gives downstream LLMs deterministic cardinality semantics (`||--o{`, `}o--o{`) instead of free-form ASCII labels.
