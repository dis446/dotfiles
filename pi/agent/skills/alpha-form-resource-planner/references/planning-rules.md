...
The core modeling rules for `alpha-form-resource-planner`. Read this while classifying entities (interview round 2) and while choosing components and actions for the Resource Map.
...
2. **Creating the `Applicant` from inside the `JobApplication` form** (e.g., a nested form, or a Save action that writes a new Applicant) — the anti-pattern. The form's first job becomes bootstrapping a person rather than collecting an application, producing duplicate Applicant records and breaking owner-based access. Establish the Applicant first; reference it from the form. See `../../alpha-form-application/references/resource-vs-form-anti-pattern.md` → "Using Resources within Forms".
...
(Access patterns — owner/role/tenant, and `access` vs `submissionAccess` — live in [access-patterns.md](access-patterns.md).)
...
- **N:N** — create a **join resource** with two `select` components (one per side). The join stores membership/relationship data and is access-neutral — access on its children is owner- (`*_own`), role-, or tenant-based, never group-based (no Group Assignment action in this stack).
...
Full component reference: see the `alpha-form-schema` skill when you need exact JSON shapes. This cheat sheet is for planning, not generation.
