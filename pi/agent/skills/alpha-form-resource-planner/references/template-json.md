...
Canonical shape for a Form.io template export on the alpha stack — byte-compatible with `default-template.json` (`title, name, version, description, roles, resources, forms, actions, access`; import via CE `POST /import` with `{ "template": <the json> }`, see `../_shared/stack.md` → Templates).

When emitting Phase B of the `alpha-form-resource-planner` skill, follow this reference verbatim. The shapes, defaults, and ordering are not negotiable — Form.io parses strictly.
...
```jsonc
{
  ...
  "description": "Optional human-readable description", // optional; immediately after version
  "roles": {
    /* role map */
  },
  "resources": {
...
**Emit these keys in EXACTLY this order, every time:** `title`, `name`, `version`, `roles`, `resources`, `forms`, `actions`, `access` — matching the alpha `default-template.json` envelope. A `description` key is optional — if you include it, place it immediately after `version` so it reads like human-facing prose at the top of the file; otherwise omit entirely. Form.io's importer is tolerant of other orderings, but downstream diff tools, grading scripts, and human reviewers key off this layout — drifting from it produces noisy diffs and gradebook failures even when the template is semantically valid. `forms`, `actions`, and `resources` may be empty objects but the keys must exist; `access` must be a non-empty array (see below).
...
- Unless the plan says otherwise, emit the default shape above verbatim. Tightening it (e.g., dropping `anonymous` from `read_all`) locks anonymous users out of the deployment's form list and usually is NOT what the user wants.
...
"<machineNameKey>": {
  ...
  "path": "url-path",              // kebab-case; served at {baseUrl}/<path> (CE direct) or {baseUrl}/v1/<path> (middleware)
...
There are **no group permissions in this stack** (no Group Assignment action — `../_shared/stack.md` → Absent); access is owner-, role-, or tenant-based. Tenant isolation is owner-based `*_own` ACLs (`tags` + `{basePath}-{orgUuid}` path scoping); team/project scoping inside an app is modeled with `*_own` plus roles, never a group action.
...
- `name`: the action type — `save`, `login`, `role`, `email`, `webhook`, etc.
- `form`: the machineName of the form/resource the action runs on
- `priority`: integer — higher runs first within the same handler phase
...
For most applications, administrator responsibilities (seeding reference data, assigning roles, inviting users, reviewing/moderating submissions) are performed by an administrator using the CE admin portal or the CE admin API (`x-token` + `API_KEYS` — see `../_shared/stack.md`). The app's login form is for end users only.
...
### Actions that do NOT exist in this stack

There is **no Group Assignment action** (`group`) in this deployment — the CE fork registers exactly six action types (`save, email, login, resetpass, role, webhook`; `../_shared/stack.md` → Actions). Never emit a `<join>:group` action, a field-based `submissionAccess` block on a select, or a hidden calculated "mirror" field; those upstream patterns have no runtime behind them here. Join resources carry plain `<name>:save` and are access-neutral — row-level access on their children is owner- (`*_own`), role-, or tenant-based, never group-based.

## Assembly checklist

Before emitting the Phase B JSON, walk through this list:

- [ ] Top-level has all keys in EXACTLY this order: `title`, `name`, `version`, `roles`, `resources`, `forms`, `actions`, `access` (`description` optional, immediately after `version`). `access` is a non-empty array (see "Top-level `access`").
- [ ] `version` is `"2.0.0"`.
...
- [ ] Every resource with user data has a meaningful `submissionAccess` reflecting the plan (owner-, role-, or public-based; no group permissions in this stack).
...
- [ ] Every login-action `settings.resources` array equals `["user"]`. If the application requires "admins" to access the application, then `"admin"` can also be included in the resources array. In most cases, however, administrator tasks are performed via the CE admin portal or CE admin API (`x-token` + `API_KEYS`), not via the app login form.
...
- [ ] No action in the template is named `group` — no Group Assignment, no field-based blocks, no mirrors (this stack has none; see "Actions that do NOT exist in this stack").
...
After writing, mentally trace one flow end-to-end: a user signs up on `userRegister` → Save action writes to `user` → Role Assignment adds `authenticated` → Login action issues a JWT → the user creates a Task (allowed by `create_own` for `authenticated` in the Task resource's `submissionAccess`) → reads and updates it (via `read_own`/`update_own`), and the owner can delete it. If any link in that chain is missing, fix before emitting — and note that the create link is the one that fails without failing the import.
