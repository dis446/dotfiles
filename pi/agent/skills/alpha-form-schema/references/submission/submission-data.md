...
`data` is the actual collected values of a submission. It is a plain JSON object whose top-level keys are the `key` properties of input components on the parent form, and whose values are the per-component stored shapes. To interpret any given `data` blob you need the parent form's `components` array (load `../form/form-definition.md` for the form envelope and `../form/input-components.md` / `../form/data-components.md` for per-component value shapes).
...
## Middleware reads — list projection and latest revision

When reading submissions through the middleware (`/v1`):

- List reads project to a fixed field set (`select=id,owner,data,created,modified`) — the full envelope (`metadata`, `access`, `roles`, …) is not returned on list reads.
- Data is returned from the **latest revision** for the `(form, requestId)` pair (see `submission-definition.md` → Middleware versioning).

## Nesting rules

Data components (the ones documented in `../form/data-components.md`) introduce nested data shapes under their own `key`:
...
For exact per-component value shapes (e.g., what a `file` component stores, what a `signature` stores, what a `select` stores when `multiple: true`), load `../form/input-components.md` and `../form/data-components.md`.
...
- `../form/form-definition.md` — the form envelope that defines the `components` array shaping `data`.
- `../form/input-components.md` — per-input-component value shapes.
- `../form/data-components.md` — per-data-component nesting shapes.
- `../form/base-component.md` — `persistent`, `clearOnHide`, `calculateValue`, `encrypted` semantics that affect what lands in `data`.
