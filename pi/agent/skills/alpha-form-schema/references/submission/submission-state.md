...
**Middleware never writes `draft`.** In the alpha stack, the submit flow writes `submitted`; the middleware's versioning layer (see `submission-definition.md`) does not create draft revisions. Drafts, where used, come from CE's own save-draft path.
...
- The `alpha-form-api` skill's `runtime-submissions` reference for how `state` interacts with the `/submission` endpoints (filtering by state, retrieving drafts, etc.).
