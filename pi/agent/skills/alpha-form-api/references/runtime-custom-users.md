...
### POST {baseUrl}/form/:employeeResourceId/action

Attach a `role` action (Role Assignment) to the custom user resource so that every new submission (registration) receives the custom role.
...
### POST {baseUrl}/:employeeResourcePath/submission
...
curl -X POST -H "Content-Type: application/json" \
  -d @employee.json \
  "{baseUrl}/employee-112/submission"
```

### POST {baseUrl}/form (Employee Login Form)
...
### PUT {baseUrl}/:employeeLoginFormPath
...
### GET {baseUrl}/:employeeLoginFormPath/action
...
curl -H "x-jwt-token: $USER_JWT" \
  "{baseUrl}/employee/login-374/action"
```

### DELETE {baseUrl}/:employeeLoginFormPath/action/:employeeLoginSaveActionId
...
### POST {baseUrl}/:employeeLoginFormPath/action
...
### POST {baseUrl}/:employeeLoginFormPath/submission
...
curl -X POST -H "Content-Type: application/json" \
  ...
  "{baseUrl}/employee/login-374/submission"
```
