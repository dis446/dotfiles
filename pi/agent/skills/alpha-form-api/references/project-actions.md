...
No MCP server exists in this deployment — use these HTTP endpoints directly. REST is the only surface. Action types are discovered dynamically via `GET .../actions` / `GET .../actions/:name` (see below); there is no separate tool catalog.

## Endpoints

### GET {baseUrl}/form/:formId/actions
...
curl -H "x-token: $API_KEY" \
  "{baseUrl}/form/69d68907040fa2cea2572b71/actions"
```

### GET {baseUrl}/form/:formId/actions/:actionName
...
```bash
curl -H "x-token: $API_KEY" \
  "{baseUrl}/form/69d68907040fa2cea2572b71/actions/email"
```

### POST {baseUrl}/form/:formId/action
...
curl -X POST -H "x-token: $API_KEY" -H "Content-Type: application/json" \
  -d @email-action.json \
  "{baseUrl}/form/69d68907040fa2cea2572b71/action"
```

### GET {baseUrl}/form/:formId/action
...
### GET {baseUrl}/form/:formId/action/:actionId
...
```bash
curl -H "x-token: $API_KEY" \
  "{baseUrl}/form/69d68907040fa2cea2572b71/action/69d6a7f7040fa2cea2572eec"
```

### PUT {baseUrl}/form/:formId/action/:actionId
...
### DELETE {baseUrl}/form/:formId/action/:actionId
...
- [form-revisions](./form-revisions.md) — form revisioning (actions themselves are not revisioned)
