...
### Environment helm values (`.gitlab/platform-1-*.yaml`)
...
```yaml
# .gitlab/env-dev.yaml  (Azure Service Bus)
application:
  env:
...
# .gitlab/env-sit.yaml  (RabbitMQ — note different port, no SSL, exchange/routing-keys)
application:
  env:
    - name: AUDIT_LOG_SOURCE
      value: "my-service"
...
| `AUDIT_LOG_AMQP_ROUTING_KEYS` | e.g. `env-name`    | not needed                           |
