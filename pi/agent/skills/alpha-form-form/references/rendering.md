...
Point `createForm` at a form under your deployment's base URL — through the middleware at `{baseUrl}/v1/form/{formPath}` (production surface), or at `{baseUrl}/{formPath}` against CE directly for admin/server ops (see [setup.md](./setup.md)):
...
Pass the form definition object directly — no server round-trip. Useful for definitions fetched via `GET {baseUrl}/v1/form/{formPath}`, stored locally, or generated at runtime:
...
const form = await Formio.createForm(
  document.getElementById('formio'),
  'https://forms.example.com/v1/form/myform/submission/{submissionId}'
);
```
