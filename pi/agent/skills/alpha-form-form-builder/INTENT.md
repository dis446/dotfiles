...
This document is loaded by the parent `alpha-form-form-builder` skill during Step 1. It is **not** a standalone skill — no frontmatter, no independent trigger.
...
**Question 1 — "What kind of form should this be?"** Two options, reordered so the inferred type is first:
...
- **Ambiguous** — a long field list with no type named. Do NOT mark any option recommended; present the two types neutrally and ask.
...
- **"No — just create the form"**, an "Other" answer that is not clearly a yes, or anything hedged ("maybe", "later", "not sure") — stash `embedIntent: no`. The flow ends at SAVE with the saved form URL. Tell the user they can embed later — the `alpha-form-form` skill picks that up whenever they ask.
...
- `formType` — `webform` | `wizard`; Step 2 (SCHEMA) passes it to `alpha-form-schema` for the definition's display mode.
