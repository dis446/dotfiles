...
### Key differences for SIT values (`.gitlab/env-sit.yaml`)
...
3. **Create `.gitlab/env-dev.yaml`**, `.gitlab/env-sit.yaml`, `.gitlab/env-test.yaml`:
   ...
   - `env-dev.yaml`: tag `$CI_COMMIT_SHA`, `AUDIT_LOG_BACKEND: "azure"` (or `"log"` if not using Azure)
   - `env-sit.yaml`: tag `${CI_COMMIT_SHA}-sit`, `AUDIT_LOG_BACKEND: "log"`
   - `env-test.yaml`: tag `$CI_COMMIT_SHA`, `AUDIT_LOG_BACKEND: "azure"`
...
6. **Push to `dev` branch** and verify pipeline runs:
   ...
   - `env-name` deploy job succeeds (if `.sit_deploy` is set up)
...
**Fix:** Ensure `env-sit.yaml`'s `tag` matches `build-sit`'s `CI_APPLICATION_TAG`.
...
When a service's `.gitlab-ci.yml` needs to be brought inline with the canonical pattern (auth / relation-store), apply this checklist. The goal is structural consistency so all env-name services are maintainable by the same mental model.
...
| 5 | **Reorder jobs** | Canonical order: deploy jobs (`env-dev`, `env-name`, `env-test`) → `build-sit` → MR test jobs (`integration-test`, `unit-test`, `sonarqube-check`, `code-style`) → `gitleaks-secret-check` → `production` → `variables`. |
...
glab ci get -R platform-1/back-end/relation-store --pipeline-id 242169
```
...
2. **Image tags must match.** The `env-sit.yaml` image tag must equal what `build-sit`'s `CI_APPLICATION_TAG` produces.
