...
description: "Use for alpha-backend CI/CD: new service setup, Dockerfile/GitLab CI/Helm values, build-vs-runtime env vars, template chain."
---
# Alpha CI/CD Expert
...
3. **Clone if not present locally**:
   ```bash
   glab repo clone internal/gitlab-ci/templates "$HOME/Code/and/internal/gitlab-ci/templates"
   ```
...
Auto-DevOps.gitlab-ci.yml  (official GitLab template — defines `build`, `production`, `.auto-deploy`)
  ↑
singlec-auto-devops.gitlab-ci.yml  (internal — legacy Helm auto-deploy; NestJS needs it PLUS the argocd template explicitly)
or
...
| Template | Use case |
|---|---|
| `singlec-auto-devops.gitlab-ci.yml` | Legacy Helm auto-deploy (`production_template` with tiller-era `auto-deploy` jobs); includes only `Auto-DevOps.gitlab-ci.yml`, no ArgoCD jobs. Good for NestJS (with the argocd template added explicitly). |
...
| `argocd-auto-devops-release.gitlab-ci.yaml` | ArgoCD deploy stages: `.dev_deploy`, `.sit_deploy`, `.test_deploy`, `.prod_deploy`, `.afs_dev_deploy`, `.uat_deploy`. Each renders Helm values, prepares chart, generates APISIX config, publishes chart. |
...
| `extract-openapi` | test | branch pushes + protected tags | Pulls OpenAPI spec from the built image (`docker cp $OPENAPI_PATH` → `openapi.yaml`) for APISIX route generation; on `prod-*` tags prefers the `${IMG}-uat` image, else the plain tag |
| `.dev_deploy` / `.sit_deploy` / `.test_deploy` / `.prod_deploy` / `.afs_dev_deploy` / `.uat_deploy` | deploy | branch pushes (`prod` only on protected `prod-*` tags) | ArgoCD Helm deploy stages |
...
|---|---|---|
| `.dev_deploy` | deploy | Renders `.gitlab/alpha-dev.yaml`, publishes chart to `$CI_REGISTRY_IMAGE/dev` |
| `.sit_deploy` | deploy | Same process, uses `.gitlab/alpha-ptf-sit.yaml`, publishes to `.../sit` |
| `.test_deploy` | deploy | Same process, uses `.gitlab/alpha-test.yaml`, publishes to `.../test` |
| `.afs_dev_deploy` | deploy | Same process, uses `.gitlab/alpha-afs-dev.yaml` (AFS tenant), publishes to `.../afs-dev` |
| `.uat_deploy` | deploy | Same process, uses `.gitlab/alpha-ptf-uat.yaml`, publishes to `.../uat` |
| `.prod_deploy` | deploy | For protected `prod-*` tag releases; reuses the image built on `$PROD_SOURCE_BRANCH` (default `main`) as `$CI_COMMIT_SHA` |
...
| `OPENAPI_PATH` | `"/openapi.yaml"` (template default; override per repo, e.g. `"/deployments/generated/openapi.yaml"` for Quarkus) | Path to OpenAPI spec inside the Docker image, consumed by `extract-openapi` (`docker cp $OPENAPI_PATH`) |
| `AUTO_DEVOPS_BUILD_IMAGE_DOCKERFILE` | `Dockerfile` | Path to Dockerfile (template default `Dockerfile` in root, e.g. `docker/Dockerfile`). Do NOT use the old `DOCKERFILE_PATH`. |
...
3. **Defines per-environment build jobs** (`build-dev`, `build-sit`, `build-test`, plus `build-uat` / `build-afs-dev` where needed) each extending `build` with environment-specific tags and `BUILD_*` variables.
4. **Deploy jobs** extend `.dev_deploy` / `.sit_deploy` / `.test_deploy` (plus `.uat_deploy` / `.afs_dev_deploy` where needed) and reference per-environment Helm values.
...
variables:
  ...
  # Only set OPENAPI_PATH if the spec is NOT at the template default /openapi.yaml:
...
application:
  secretName: "my-service-secret"
  env:
    # Database uses the env's internal cluster IP, not Azure (example value below)
    - name: DB_HOST
      value: "10.5.110.11"
    - name: DB_PORT
      value: "5432"
    ...
    - name: AUDIT_LOG_AMQP_PASSWORD
      value: "$AUDIT_LOG_AMQP_PASSWORD"
    # SIT typically omits OTEL runtime env (no OTEL_ENDPOINT etc.); Sonar runs on MRs only, never via deploy values
...
> UAT (`build-uat` → `alpha-uat` → `.uat_deploy`, values `.gitlab/alpha-ptf-uat.yaml`, tag `${CI_COMMIT_SHA}-uat`) follows the same shape. Note UAT promotes through Harbor (`harbor-uat.*/...-uat` images), not `reg.git.and.global`, and prod tags reuse the `$PROD_SOURCE_BRANCH` (default `main`) image with a `-uat` fallback for `extract-openapi`.
...
NestJS projects use `singlec-auto-devops.gitlab-ci.yml` (legacy Helm auto-deploy including only `Auto-DevOps.gitlab-ci.yml`, no ArgoCD jobs — hence the second include below). They typically don't pass OTEL/AUDIT build args to Docker since NestJS uses environment variables at runtime, not Maven properties.
...
variables:
  ...
  AUTO_DEVOPS_BUILD_IMAGE_DOCKERFILE: docker/Dockerfile
...
2. **Create `.gitlab-ci.yml`** using the canonical pattern above. Adjust:
   ...
   - `OPENAPI_PATH` — only if the spec is not at the template default `/openapi.yaml` (e.g. Quarkus `"/deployments/generated/openapi.yaml"`)
...
5. **Ensure the `image.repository` path matches** the GitLab project path (dev/sit charts):
   ```
   reg.git.and.global:443/alpha/back-end/<service-name>/dev
   ```
   UAT deploys promote through Harbor instead — expect `harbor-uat.*` image references there, not `reg.git.and.global`.
...
| 7 | **Replace `build-sit`** with per-environment builds (`build-dev`, `build-sit`, `build-test`, plus `build-uat` / `build-afs-dev` where needed) | Each gets its own tag suffix and `BUILD_*` overrides. `build-dev` and `build-sit` both trigger on `dev`; `build-test`/`build-uat` trigger on `main`. |
| 8 | **Update deploy jobs** to reference per-environment tags | `alpha-dev.yaml` uses `${CI_COMMIT_SHA}-dev`, `alpha-ptf-sit.yaml` uses `${CI_COMMIT_SHA}-sit`, `alpha-test.yaml` uses `${CI_COMMIT_SHA}-test` (plus `-uat` / `-afs-dev` equivalents where needed). |
...
6. **The `extract-openapi` job** (stage `test`) pulls `$CI_APPLICATION_REPOSITORY:$CI_APPLICATION_TAG` and `docker cp`s `$OPENAPI_PATH` to `openapi.yaml`. Deploy jobs consume it via `needs: [extract-openapi]`. On protected `prod-*` tags it prefers the `${IMG}-uat` image (build doesn't run on tags). All per-env builds produce the same spec, so any variant's image is safe.
...
12. **RabbitMQ is in-cluster only — a CI runner cannot reach it.** RabbitMQ always runs inside the k8s cluster (`rabbitmq.rabbitmq.svc.cluster.local`); only **one** environment exposes it to the public internet and the others do not. So you cannot sync definitions/queues from a GitLab CI runner. The pattern is a **k8s Init Container** (curl alone is enough for an HTTP sync) which the cluster gives the pod's secrets, performs the sync, and exits. Do not plan CI-side RabbitMQ admin calls.
