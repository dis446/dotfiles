---
name: env-name
description: Deploy, diagnose, and fix CI/CD for any env-name service (NestJS and Quarkus). Covers new microservice setup, existing repo troubleshooting, template chain understanding, build-vs-runtime variable distinctions, and ArgoCD Helm deployments.
---
...
This skill covers the **env-name** CI/CD ecosystem at AND Global:
...
The CI templates repo is at `internal/gitlab-ci/templates` on `<gitlab-host>`. It **must not** be hardcoded by absolute path because it needs to work on any developer machine.

**Algorithm to find it:**

1. **Check common locations** under the user's `Code/<org>/` workspace:
   - `$HOME/Code/<org>/internal/gitlab-ci/templates`
...
3. **Clone if not present locally**:
   ```bash
   glab repo clone internal/gitlab-ci/templates --dir "$HOME/Code/<org>/internal/gitlab-ci/templates"
   ```
...
|---|---|---|
| `.dev_deploy` | deploy | Pulls image from `build` job's dotenv (`CI_APPLICATION_TAG`), renders `.gitlab/env-dev.yaml` |
| `.sit_deploy` | deploy | Same process, uses `.gitlab/env-sit.yaml` |
| `.test_deploy` | deploy | Same process, uses `.gitlab/env-test.yaml` |
...
### Runtime variables (set in Helm values e.g. `env-dev.yaml`)
...
| **Runtime** | When the pod starts | `env-dev.yaml` → Kubernetes `env:` → Quarkus config property | Controls which backend bean is activated at startup |
...
env-dev:
  extends: ".dev_deploy"
  variables:
    VALUES_FILE: ".gitlab/env-dev.yaml"
  rules:
    - if: '$CI_COMMIT_BRANCH == "dev"'
  environment:
    name: env-dev
...
env-name:
  extends: .sit_deploy
  variables:
    VALUES_FILE: ".gitlab/env-sit.yaml"
  rules:
    - if: '$CI_COMMIT_BRANCH == "dev"'
  environment:
    name: env-sit
...
env-test:
  extends: ".test_deploy"
  variables:
    VALUES_FILE: ".gitlab/env-test.yaml"
  rules:
    - if: '$CI_COMMIT_BRANCH == "main"'
  environment:
    name: env-test
...
### Helm values pattern (`.gitlab/env-dev.yaml`)
...
application:
  ...
  env:
    ...
    - name: DB_SCHEMA
      value: "my_service"
    - name: DB_HOST
      value: "postgres-env-dev-03.postgres.database.azure.com"
    - name: DB_PORT
      value: "6432"
    ...
    - name: AUDIT_LOG_AMQP_HOST
      value: "sb-env-dev-02.servicebus.windows.net"
...
image:
  repository: reg.<gitlab-host>:443/platform-1/back-end/my-service/dev
...
### SIT values pattern (`.gitlab/env-sit.yaml`)
...
### Test values pattern (`.gitlab/env-test.yaml`)

Same shape as dev but with test environment endpoints:

```yaml
image:
  repository: reg.<gitlab-host>:443/platform-1/back-end/my-service/main  # main branch
...
2. **Create `.gitlab-ci.yml`** using the canonical pattern above. Adjust:
   ...
   - Deploy jobs (`env-dev`, `env-name`, `env-test`) — set correct branch rules
   - Add `gitleaks-secret-check` job
   - Add `production` dummy job that `when: never`

3. **Create `.gitlab/env-dev.yaml`**, `.gitlab/env-sit.yaml`, `.gitlab/env-test.yaml`:
   - Set correct DB credentials, image tag, ports
   - `env-dev.yaml`: tag `${CI_COMMIT_SHA}-dev`, `AUDIT_LOG_BACKEND: "amqp"` (or `"azure"`), with OTEL endpoints, ingress, health probes, pull secrets
   - `env-sit.yaml`: tag `${CI_COMMIT_SHA}-sit`, `AUDIT_LOG_BACKEND: "log"`, no ingress, internal DB
   - `env-test.yaml`: tag `${CI_COMMIT_SHA}-test`, same shape as dev with test endpoints
...
6. **Ensure the `image.repository` path matches** the GitLab project path:
   ```
   reg.<gitlab-host>:443/platform-1/back-end/<service-name>/dev
   ```
...
7. **Push to `dev` branch** and verify pipeline runs:
   ...
   - `env-dev` deploy job succeeds
   - `env-name` deploy job succeeds
...
- `env-sit.yaml` image tag doesn't match what `build-sit` produces
...
grep "tag:" .gitlab/env-sit.yaml
```

**Fix:** Ensure `env-sit.yaml`'s `tag` matches `build-sit`'s `CI_APPLICATION_TAG`. In the canonical pattern, both use `${CI_COMMIT_SHA}-sit`.
...
# Check runtime value
grep "AUDIT_LOG_BACKEND" .gitlab/env-sit.yaml
```
...
- `.gitlab/env-sit.yaml`: set `AUDIT_LOG_BACKEND: "log"` in the env section
...
grep "tag:" .gitlab/platform-1-*.yaml
...
- `build-dev` → `env-dev.yaml`: `${CI_COMMIT_SHA}-dev`
- `build-sit` → `env-sit.yaml`: `${CI_COMMIT_SHA}-sit`
- `build-test` → `env-test.yaml`: `${CI_COMMIT_SHA}-test`
...
When a service's `.gitlab-ci.yml` needs to be brought inline with the canonical Quarkus pattern, apply this checklist. The goal is structural consistency so all env-name Quarkus services are maintainable by the same mental model.
...
| 8 | **Update deploy jobs** to reference per-environment tags | `env-dev.yaml` uses `${CI_COMMIT_SHA}-dev`, `env-sit.yaml` uses `${CI_COMMIT_SHA}-sit`, `env-test.yaml` uses `${CI_COMMIT_SHA}-test`. |
...
After refactoring `.gitlab-ci.yml`, verify `.gitlab/env-sit.yaml`:
...
glab ci get -R platform-1/back-end/my-service --pipeline-id 242169
```
...
2. **Image tags must match per environment.** The `env-sit.yaml` image tag must equal what `build-sit`'s `CI_APPLICATION_TAG` produces (`${CI_COMMIT_SHA}-sit`).
