---
name: env-name
description: Set up, audit, and standardize CI/CD configs for any env-name service (NestJS and Quarkus). Covers new microservice setup, existing repo config review, template chain understanding, build-vs-runtime variable distinctions, and canonical patterns for Dockerfiles, GitLab CI, and Helm values.
---
...
|---|---|---|---|---|
| Dev | `sb-env-dev-02.servicebus.windows.net` | `5671` | `true` | Azure Service Bus, needs `app_log_mgmt` user/pass |
...
3. **Create `.gitlab/env-dev.yaml`**, `.gitlab/env-sit.yaml`, `.gitlab/env-test.yaml`:
   ...
   - `env-sit.yaml`: tag `${CI_COMMIT_SHA}-sit`, `AUDIT_LOG_BACKEND: "amqp"`, no ingress, internal DB
